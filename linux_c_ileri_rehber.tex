...
\begin{lstlisting}
// sayac = 41 iken iki iplik ayni anda sayac++ yaparsa:
...
_Atomic long sayac = 0;             // artik ++ atomik: yaris yok

void *artir(void *arg) {
    (void)arg;
    for (int i = 0; i < 1000000; i++)
        sayac++;                    // lock xadd -> bolunemez oku-artir-yaz
    return NULL;
}
int main(void) {
    pthread_t a, b;
...
int eski = atomic_fetch_add(&x, 5);      // x -> 15, eski = 10
...
void atomik_ekle(atomic_int *hedef, int deger) {
    int beklenen = atomic_load(hedef);
    int yeni;
    do {
        yeni = beklenen + deger;
        // hedef hala 'beklenen' ise 'yeni'yi yaz; degilse beklenen guncellenir
    } while (!atomic_compare_exchange_weak(hedef, &beklenen, yeni));
}
\end{lstlisting}
...
typedef struct Dugum { int veri; struct Dugum *next; } Dugum;
_Atomic(Dugum *) tepe = NULL;            // atomik isaretci

void push(int v) {
    Dugum *yeni = malloc(sizeof(Dugum));
    yeni->veri = v;
    yeni->next = atomic_load(&tepe);
    // tepe hala yeni->next ise yeni'yi tepe yap; degilse yeni->next tazelenir
    while (!atomic_compare_exchange_weak(&tepe, &yeni->next, yeni))
        ;                                // bos govde: beklemeden yeniden dene
}

int pop(int *out) {                      // 1: basarili, 0: bos
    Dugum *eski = atomic_load(&tepe);
    while (eski && !atomic_compare_exchange_weak(&tepe, &eski, eski->next))
        ;                                // eski, guncel tepeyle tazelenir
    if (!eski) return 0;
    *out = eski->veri;
    free(eski);                          // NOT: ABA problemine dikkat (asagi)
    return 1;
}
\end{lstlisting}
...
atomic_long islem_sayisi = 0;

void olay(void) {
    // Sadece toplam onemli; hangi sirada arttigi onemsiz -> relaxed
    atomic_fetch_add_explicit(&islem_sayisi, 1, memory_order_relaxed);
}
\end{lstlisting}
...
int      veri = 0;                       // sıradan degisken
atomic_bool hazir = false;

void uretici(void) {
    veri = 42;                                          // (1) once veriyi yaz
    atomic_store_explicit(&hazir, true,
                          memory_order_release);        // (2) sonra bayragi set et
}
void tuketici(void) {
    while (!atomic_load_explicit(&hazir, memory_order_acquire))
        ;                                               // bayragi bekle
...
atomic_flag kilit = ATOMIC_FLAG_INIT;

void kilitle(void) {
    // set edip eski deger true ise baskasi tutuyor demektir -> don ve bekle
    while (atomic_flag_test_and_set_explicit(&kilit, memory_order_acquire))
        ;                                // "busy-wait" (mesgul bekleme)
}
void ac(void) {
    atomic_flag_clear_explicit(&kilit, memory_order_release);
}
\end{lstlisting}
...
\begin{lstlisting}
long sayac = 0;                          // _Atomic gerekmez

__atomic_fetch_add(&sayac, 1, __ATOMIC_SEQ_CST);   // C11 fetch_add karsiligi
long v = __atomic_load_n(&sayac, __ATOMIC_ACQUIRE);
__atomic_store_n(&sayac, 0, __ATOMIC_RELEASE);

// Eski __sync ailesi (her zaman tam bariyer):
__sync_fetch_and_add(&sayac, 1);
__sync_bool_compare_and_swap(&sayac, 0, 100);      // CAS
\end{lstlisting}
