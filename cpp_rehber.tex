...
% #####################################################################
\gpart{12}{Bellek Optimizasyonu: Yanlışlar ve Doğruları}
% #####################################################################

Bellek optimizasyonu, C++'ın en çok ``sezgiyle'' yapılıp en çok yanlış
yapılan alanıdır. Çoğu program yavaşlar; çünkü hesaplama pahalı değil,
\emph{bellek erişimi} pahalıdır: gereksiz tahsisler (allocation), gizli
kopyalar, önbellek (cache) ıskalamaları ve parçalanmış (fragmented) bellek
düzeni. Bu kısım, en yaygın hataları \textbf{``Yanlış'' / ``Doğru''}
çiftleri hâlinde gösterir. Her örnekte önce naif (ve genelde çalışan ama
yavaş) sürümü, ardından ölçülebilir biçimde daha hızlı doğru sürümü
göreceksiniz.

\bilgi{\textbf{Optimizasyonun altın kuralı: önce ölç.} Sezgiye güvenmeyin;
darboğazı bir profilleyici (profiler) ile bulun. Bellek için:
\code{valgrind -{}-tool=massif} (öbek kullanımı), \code{heaptrack} (tahsis
sayısı ve kaynağı), \code{perf stat -e cache-misses} (önbellek ıskalaması).
Optimize ettiğiniz her şeyi \emph{öncesi/sonrası} ölçün. ``Erken
optimizasyon'' kadar zararlı olan tek şey, \emph{ölçmeden} optimizasyondur.}

% =====================================================================
\gsec{Gereksiz Kopyalar ve Yeniden Tahsisler}

En sık ve en pahalı hata, \code{std::vector}'ün kaç eleman alacağını bilerek
ama yine de onu boş başlatıp tek tek büyütmektir. \code{vector} dolunca
kapasitesini genellikle iki katına çıkarır: \emph{yeni} bir bellek bloğu
ayırır, tüm mevcut elemanları oraya taşır/kopyalar ve eskisini serbest
bırakır. $N$ eleman eklerken bu, $\log_2 N$ kez yeniden tahsis ve toplam
$\mathcal{O}(N)$ ekstra kopya demektir.

\gsub{Yanlış: reserve'süz büyüyen vektör}

\begin{lstlisting}
#include <vector>
#include <string>

std::vector<std::string> build_wrong(int n) {
    std::vector<std::string> result;          // kapasite = 0
    for (int i = 0; i < n; ++i)
        result.push_back("oge-" + std::to_string(i));
    // n=1'000'000 icin: vektor ~20 kez yeniden tahsis edilir ve
    // her seferinde TUM mevcut string'ler yeni bloga TASINIR.
    return result;
}
\end{lstlisting}

\gsub{Doğru: kapasiteyi önceden ayır (reserve) + emplace}

\begin{lstlisting}
#include <vector>
#include <string>

std::vector<std::string> build_right(int n) {
    std::vector<std::string> result;
    result.reserve(n);                        // TEK tahsis: n elemanlik yer
    for (int i = 0; i < n; ++i)
        result.emplace_back("oge-" + std::to_string(i));
        // emplace_back: string'i dogrudan vektorun icinde INSA eder,
        // gecici bir string olusturup KOPYALAMAZ (push_back bunu yapabilir)
    return result;
}
\end{lstlisting}

\notk[Ölçüm]{1 milyon elemanlık bir \code{vector<string>} için
\code{reserve} eklemek, tipik olarak yeniden tahsis sayısını $\sim$20'den
\textbf{1'e} indirir ve toplam string taşımalarını sıfıra yaklaştırır ---
çoğu makinede $2\times$--$4\times$ hızlanma. \code{reserve}, boyutu
(\code{size}) değil kapasiteyi (\code{capacity}) değiştirir; elemanları
yine \code{push\_back}/\code{emplace\_back} ile eklersiniz.}

\gsub{push\_back vs emplace\_back}

\code{push\_back(T(...))} önce geçici bir \code{T} nesnesi kurar, sonra onu
konteynere \emph{kopyalar} veya \emph{taşır}. \code{emplace\_back(...)} ise
argümanları doğrudan konteynerin belleğine geçirip nesneyi \emph{yerinde}
(in place) kurar --- geçici nesne ve fazladan kopya/taşıma yok.
...
int main() {
    ...
    pts.push_back(Point(1, 2));   // 1) gecici Point kur  2) vektore TASI
    pts.push_back({3, 4});        // yine gecici + tasima
    pts.emplace_back(5, 6);       // dogrudan vektorde INSA -- gecici yok

    return 0;
}
\end{lstlisting}

\ipucu{Varsayılan olarak \code{emplace\_back} tercih edin; en kötü ihtimalle
\code{push\_back} kadar hızlıdır. Ancak yalnızca hazır bir nesneyi
ekliyorsanız (\code{v.push\_back(std::move(mevcut))}) ikisi de aynıdır ---
``emplace her zaman daha hızlıdır'' bir efsanedir, fark yalnızca \emph{geçici
nesne kurulumundan} kaçınıldığında ortaya çıkar.}

\gsub{Yanlış: döngüde gizli kopya (range-for)}

\begin{lstlisting}
#include <vector>
#include <string>

long total_length_wrong(const std::vector<std::string>& lines) {
    long total = 0;
    for (auto line : lines)      // HATA: her eleman KOPYALANIR (string tahsisi!)
        total += line.size();    // sadece boyutu okuyacaktik, kopya bosuna
    return total;
}
\end{lstlisting}

\gsub{Doğru: sabit referansla gez, kopyayı kaldır}

\begin{lstlisting}
#include <vector>
#include <string>

long total_length_right(const std::vector<std::string>& lines) {
    long total = 0;
    for (const auto& line : lines)   // const ref: KOPYA YOK, dogrudan eleman
        total += line.size();
    return total;
}
\end{lstlisting}

\uyari{\code{for (auto x : kap)} biçimi, konteynerin her elemanını
\emph{kopyalar}. Elemanlar \code{std::string}, \code{std::vector} ya da büyük
sınıflarsa bu, döngünün her turunda bir öbek tahsisi + kopya demektir.
Elemanı değiştirmeyecekseniz \textbf{her zaman} \code{const auto\&},
değiştirecekseniz \code{auto\&} kullanın. Salt \code{auto} yalnızca
\code{int} gibi ucuz, kopyalanması bedava tipler için uygundur.}

% =====================================================================
\gsec{String'ler: Tahsis, Birleştirme ve string\_view}

String'ler, gizli tahsislerin bir numaralı kaynağıdır. Her \code{std::string}
kısa metinler için genellikle öbek kullanmaz (aşağıdaki SSO), ama uzun
metinler, birleştirmeler ve alt dizeler (\code{substr}) her seferinde yeni
bir öbek bloğu tahsis eder.

\gsub{Yanlış: operator+ zinciriyle birleştirme}

\begin{lstlisting}
#include <string>
#include <vector>

std::string join_wrong(const std::vector<std::string>& parts) {
    std::string out;
    for (const auto& p : parts)
        out = out + p + ", ";
        // HER turda: (out + p) yeni bir GECICI string tahsis eder,
        // sonra (+", ") bir GECICI DAHA, sonra out'a atanir.
        // n parca icin O(n^2) karakter kopyasi + 2n tahsis.
    return out;
}
\end{lstlisting}

\gsub{Doğru: reserve + += ile yerinde ekle}
...
std::string join_right(const std::vector<std::string>& parts) {
    // 1) Toplam uzunlugu onceden hesapla, TEK tahsis yap
    std::size_t need = 0;
    for (const auto& p : parts) need += p.size() + 2;

    std::string out;
    out.reserve(need);                 // tek seferde yeterli yer

    for (const auto& p : parts) {
        out += p;                      // yerinde ekle: gecici yok
        out += ", ";                   // += operator+'nin aksine kopyalamaz
    }
    return out;                        // NRVO ile kopyasiz doner
}
\end{lstlisting}

\notk[Neden bu kadar fark eder?]{\code{a = a + b} ifadesi \code{a} ve
\code{b}'nin toplamını tutacak \emph{yeni} bir geçici string tahsis eder,
sonra onu \code{a}'ya atar --- eski tampon çöpe gider. \code{a += b} ise
\code{a}'nın mevcut tamponunda yer varsa hiç tahsis yapmadan sona ekler.
Bir döngüde bu fark, $\mathcal{O}(n^2)$'lik bir kopya patlamasını
$\mathcal{O}(n)$'e indirir.}

\gsub{Yanlış: salt okunur parametre için gereksiz kopya/tahsis}

\begin{lstlisting}
#include <string>
#include <cctype>

// Bu fonksiyon parametreyi degistirmez, sadece OKUR.
// Ama std::string alir: cagirinca cogu zaman bir KOPYA/tahsis olur.
bool starts_with_wrong(std::string text, char c) {   // deger -> kopya
    return !text.empty() && text.front() == c;
}
// starts_with_wrong(cok_uzun_string, 'x');  // koca string kopyalanir
// starts_with_wrong("literal", 'x');        // literal -> gecici string tahsisi
\end{lstlisting}

\gsub{Doğru: string\_view ile sahiplenmeden, tahsissiz oku}

\begin{lstlisting}
#include <string>
#include <string_view>

// string_view: bir karakter dizisine (isaretci + uzunluk) sahiplenmeyen
// bir "pencere". KOPYALAMAZ, TAHSIS ETMEZ; sadece var olan veriyi gosterir.
bool starts_with_right(std::string_view text, char c) {
    return !text.empty() && text.front() == c;
}
// Hepsi TAHSISSIZ calisir:
//   starts_with_right(cok_uzun_string, 'x');  // sadece pencere olusur
//   starts_with_right("literal", 'x');        // literal dogrudan gorunur
//   starts_with_right(text.substr(...) ...)   // DIKKAT: substr yerine
//   text_view.substr(...) kullan -- o da tahsissizdir
\end{lstlisting}

\uyari{\code{std::string\_view} güçlüdür ama \textbf{sahiplenmez}: yalnızca
başka birinin belleğine bakan bir işaretçi + uzunluktur. Gösterdiği string
yok olursa \code{string\_view} \emph{sarkan} (dangling) hâle gelir. Asla bir
geçici string'in \code{string\_view}'ını saklamayın
(\code{std::string\_view sv = get\_string();} --- \code{get\_string()}'in
dönüşü hemen ölür!). Salt okunur \emph{parametreler} için idealdir; üye
olarak saklarken ömrüne çok dikkat edin.}

\gsub{Küçük String Optimizasyonu (SSO)}
...
int main() {
    std::string kisa = "merhaba";        // ~15 karaktere kadar: OBEK YOK,
                                         // karakterler string nesnesinin
                                         // ICINDE (stack'te) saklanir -> SSO
    std::string uzun(1000, 'x');         // uzun: OBEK tahsisi yapilir

    // SSO sayesinde kisa string'ler bedava kopyalanir; bu yuzden kucuk
    // string'lerde asiri optimizasyon (mesela her yerde string_view)
    // gereksiz olabilir -- once OLC.
    std::cout << kisa.capacity() << "\n"; // tipik: 15 (libstdc++)
    return 0;
}
\end{lstlisting}

\bilgi{\textbf{SSO (Small String Optimization):} Modern standart kütüphaneler,
kısa string'leri (genelde $\leq 15$ karakter) öbek yerine string nesnesinin
kendi içinde saklar. Bu yüzden kısa string kopyaları neredeyse bedavadır ve
her string parametresini körü körüne \code{string\_view}'a çevirmek her
zaman kazanç sağlamaz. Kazanç, \emph{uzun} string'lerde ve \emph{çok sayıda}
kopyada belirginleşir.}

% =====================================================================
\gsec{Taşıma Semantiğini Kaçırmak ve Yanlış std::move Kullanımı}

Kısım~5'te taşıma semantiğini gördük; burada onu \emph{optimizasyon} açısından
en sık suistimal edilen iki hatayla ele alalım: (1) taşınabilecek yerde
kopyalamak, (2) derleyicinin zaten yapacağı optimizasyonu \code{std::move}
ile \emph{engellemek}.

\gsub{Yanlış: dönüşte std::move (RVO'yu bozar)}

\begin{lstlisting}
#include <vector>

std::vector<int> make_wrong() {
    std::vector<int> v(1000, 7);
    return std::move(v);
    // HATA: yerel degiskeni std::move ile dondurmek, derleyicinin
    // NRVO (Named Return Value Optimization) ile nesneyi cagiranin
    // yerinde DOGRUDAN insa etmesini ENGELLER. std::move burada
    // hem gereksiz hem ZARARLI: en iyi ihtimalle bir tasima ekler,
    // RVO olsaydi SIFIR is olurdu.
}
\end{lstlisting}

\gsub{Doğru: yerel değişkeni olduğu gibi döndür}

\begin{lstlisting}
#include <vector>

std::vector<int> make_right() {
    std::vector<int> v(1000, 7);
    return v;          // NRVO: derleyici v'yi cagiranin yerinde insa eder,
                       // hicbir kopya/tasima olmaz (copy elision)
}
\end{lstlisting}

\uyari{Bir fonksiyondan \emph{yerel bir değişkeni} döndürürken asla
\code{return std::move(local);} yazmayın. Derleyici zaten kopya seçimini
(copy elision / RVO) uygular ve nesneyi doğrudan çağıranın belleğinde kurar.
\code{std::move} eklemek bu optimizasyonu kapatır ve gereksiz bir taşıma
ekler. \emph{İstisna:} döndürdüğünüz şey bir üye ya da parametre ise
(yerel değil) taşıma bazen yerinde olabilir.}

\gsub{Yanlış / Doğru: değere alıp saklarken}
...
public:
    // YANLIS: const ref alip sonra KOPYALAMAK. Cagiran gecici verse bile
    // bu kod her zaman kopyalar.
    // Widget(const std::string& n) : name_(n) {}   // hep kopya

    // DOGRU: degere al + std::move ile ICERI TASI.
    // Cagiran lvalue verirse 1 kopya, rvalue (gecici) verirse 0 kopya.
    Widget(std::string n, std::vector<int> d)
        : name_(std::move(n)), data_(std::move(d)) {}
        // parametreler YEREL kopyalardir; onlari uyeye TASImak dogrudur
        // (burada std::move dogru kullanim -- RVO devrede degil)
};

int main() {
    std::vector<int> big(10000, 1);
    Widget w("gecici", std::move(big));   // big -> parametreye tasinir,
                                          // parametre -> uyeye tasinir. Kopya yok.
    return 0;
}
\end{lstlisting}

\bilgi{\textbf{``Değere al, sonra taşı'' (sink parameter) deyimi:} Bir
kurucunun/fonksiyonun argümanı \emph{saklayacağı} durumlarda parametreyi
\emph{değere} alıp üyeye \code{std::move} ile taşıyın. Bu tek imza, hem
lvalue (bir kopya) hem rvalue (sıfır kopya) çağrılarında optimaldir ve
\code{const T\&} + \code{T\&\&} aşırı yüklemesi yazma zahmetinden kurtarır.
Kural şudur: \emph{yerel değişkeni/parametreyi} taşıyın (\code{std::move}
doğru), \emph{dönüş değerini} taşımayın (derleyiciye bırakın).}

% =====================================================================
\gsec{Bellek Düzeni ve Önbellek Dostu Kod}

Modern işlemcilerde bir L1 önbellek erişimi $\sim$1 ns iken, ana bellekten
(RAM) okuma $\sim$100 ns'dir --- yaklaşık \textbf{100 kat} fark. Bu yüzden
verinin \emph{düzeni}, çoğu zaman algoritmanın karmaşıklığından daha çok
performansı belirler. Bitişik (contiguous) ve önbellek satırına (cache line,
tipik 64 bayt) sığan veri hızlıdır; parçalanmış, işaretçilerle dolaşılan
veri yavaştır.

\gsub{Yanlış: dağınık düğümler (list / vector-of-pointers)}
...
// std::list: her dugum AYRI bir obek tahsisi, bellekte DAGINIK.
// Ustunden gecmek her adimda bir onbellek iskalamasi (cache miss) demektir.
long sum_wrong(const std::list<int>& lst) {
    long s = 0;
    for (int x : lst) s += x;   // her x muhtemelen farkli bir cache line'da
    return s;
}

// Ayni sorun: vector<unique_ptr<T>> -- isaretciler bitisik ama
// GOSTERDIKLERI nesneler obekte dagInIk.
long sum_ptrs_wrong(const std::vector<std::unique_ptr<int>>& v) {
    long s = 0;
    for (const auto& p : v) s += *p;   // her *p bir dolayli erisim + cache miss
    return s;
}
\end{lstlisting}

\gsub{Doğru: bitişik dizi (contiguous vector)}

\begin{lstlisting}
#include <vector>

// std::vector<int>: tum elemanlar ARDISIK bellekte. Islemci veriyi
// onbellek satirlari halinde onceden getirir (prefetch); dolasim cok hizli.
long sum_right(const std::vector<int>& v) {
    long s = 0;
    for (int x : v) s += x;   // ardisik erisim -> neredeyse hep cache HIT
    return s;
}
\end{lstlisting}

\notk[Ölçüm]{Aynı sayıda \code{int} için \code{std::vector} üzerinde toplam
almak, \code{std::list} üzerinde almaktan tipik olarak
\textbf{$5\times$--$30\times$} daha hızlıdır --- algoritma ikisinde de
$\mathcal{O}(n)$ olmasına rağmen. Fark tamamen bellek düzeninden ve önbellek
davranışından gelir. ``Ortadan sık ekleme/silme'' gerçekten baskın değilse
\code{list} yerine \code{vector} seçin.}

\gsub{AoS ve SoA: sıcak/soğuk alan ayrımı}

Bir yapının yalnızca \emph{birkaç} alanını sıkça işliyorsanız, tüm yapıyı
(kullanılmayan alanlarıyla) önbelleğe taşımak bant genişliğini boşa harcar.
\textbf{AoS} (Array of Structs) yerine \textbf{SoA} (Struct of Arrays)
düzeni, yalnızca ihtiyaç duyulan alanları bitişik tutar.

\begin{lstlisting}
#include <vector>
#include <string>

// YANLIS icin de dogru icin de senaryo: 1 milyon parcacigin sadece
// pozisyonunu her karede guncelliyoruz; isim/renk nadiren kullaniliyor.

// --- AoS: Array of Structs (naif) ---
struct ParticleAoS {
    float x, y, z;          // SICAK: her kare guncellenir (12 bayt)
    float vx, vy, vz;       // SICAK (12 bayt)
    std::string name;       // SOGUK: nadir kullanilir ama her nesnede DURUR
    unsigned color;         // SOGUK
};
void update_aos(std::vector<ParticleAoS>& ps, float dt) {
    for (auto& p : ps) {    // her p ~40+ bayt -> onbellege SOGUK alanlar da gelir
        p.x += p.vx * dt; p.y += p.vy * dt; p.z += p.vz * dt;
    }
}

// --- SoA: Struct of Arrays (onbellek dostu) ---
struct ParticlesSoA {
    std::vector<float> x, y, z;      // SICAK alanlar ayri, BITISIK diziler
    std::vector<float> vx, vy, vz;
    std::vector<std::string> name;   // SOGUK: ayri, sicak dongu bunlara DOKUNMAZ
    std::vector<unsigned> color;
};
void update_soa(ParticlesSoA& ps, float dt) {
    const std::size_t n = ps.x.size();
    for (std::size_t i = 0; i < n; ++i) {   // sadece sicak diziler taranir
        ps.x[i] += ps.vx[i] * dt;
        ps.y[i] += ps.vy[i] * dt;
        ps.z[i] += ps.vz[i] * dt;           // onbellek satirlari %100 verimli
    }
}
\end{lstlisting}

\ipucu{\textbf{Sıcak/soğuk ayrımı:} Bir döngü bir yapının yalnızca birkaç
alanına dokunuyorsa, o alanları (sıcak) diğerlerinden (soğuk) ayırın. SoA
düzeni, SIMD vektörleştirmeyi de kolaylaştırır: derleyici \code{x[i]},
\code{x[i+1]}, \dots{} üzerinde tek talimatla çalışabilir. AoS ise kodu
yazması ve ``bir nesne'' olarak düşünmesi daha kolaydır; ölçüm SoA'yı
haklı çıkarmadıkça AoS ile başlayın.}

\gsub{Yapı Hizalama ve Dolgu (Padding)}

Derleyici, her üyeyi kendi hizasına (alignment) yerleştirmek için üyeler
arasına görünmez \emph{dolgu} (padding) baytları ekler. Üye sırasını yanlış
seçmek, yapıyı gereksiz yere büyütür --- yani daha az nesne önbelleğe sığar.

\begin{lstlisting}
#include <cstdint>
#include <iostream>

// YANLIS sira: bol dolgu (padding)
struct BadLayout {
    char  a;      // 1 bayt + 7 dolgu (double'i hizalamak icin)
    double b;     // 8 bayt
    char  c;      // 1 bayt + 7 dolgu (yapiyi 8'e hizalamak icin)
};                // toplam: 24 bayt (14 bayti VERI, 10 bayti COP)

// DOGRU sira: buyukten kucuge diz -> dolgu en aza iner
struct GoodLayout {
    double b;     // 8 bayt
    char  a;      // 1 bayt
    char  c;      // 1 bayt + 6 dolgu
};                // toplam: 16 bayt

int main() {
    std::cout << sizeof(BadLayout)  << "\n";   // 24
    std::cout << sizeof(GoodLayout) << "\n";   // 16
    // 1 milyon nesnede: 24 MB yerine 16 MB -> %33 az bellek,
    // %33 daha fazla nesne ayni onbellege sigar.
    return 0;
}
\end{lstlisting}

\notk[Kural]{Yapı üyelerini genellikle \textbf{büyükten küçüğe} sıralayın
(\code{double}/işaretçi $\to$ \code{int} $\to$ \code{short} $\to$ \code{char}).
Bu, dolgu baytlarını en aza indirir. Boyutu doğrulamak için
\code{sizeof} ve \code{alignof} kullanın; kritik veri yapılarında bu basit
yeniden sıralama, bellek ayak izini görünür biçimde düşürür ve önbellek
verimini artırır.}

% =====================================================================
\gsec{Tahsis Maliyetleri: shared\_ptr, make\_shared ve Nesne Havuzları}

Öbek tahsisi (\code{new}/\code{malloc}) ucuz değildir: kilit alma, uygun blok
arama, defter tutma içerir --- tipik olarak yüzlerce nanosaniye. Çok sayıda
küçük tahsis, hem yavaştır hem belleği parçalar (fragmentation). Bu bölümde
tahsis sayısını azaltmanın yollarını görelim.

\gsub{Yanlış: iki tahsis (shared\_ptr + new)}
...
int main() {
    // HATA: IKI ayri tahsis yapar --
    //   1) new ile Object icin obek blogu
    //   2) shared_ptr'in kontrol blogu (referans sayaci) icin AYRI blok
    std::shared_ptr<std::string> a(new std::string("merhaba"));
    return 0;
}
\end{lstlisting}

\gsub{Doğru: make\_shared (tek tahsis)}
...
int main() {
    // DOGRU: make_shared, nesneyi VE kontrol blogunu TEK bir obek
    // tahsisinde birlestirir. Daha hizli, daha az parcalanma, istisna guvenli.
    auto a = std::make_shared<std::string>("merhaba");
    return 0;
}
\end{lstlisting}

\uyari{\code{shared\_ptr}'ı gereğinden fazla kullanmak yaygın bir hatadır.
Her kopya, iş parçacığı güvenliği için \emph{atomik} bir sayaç artışı/azalışı
yapar --- bu, \code{unique\_ptr}'a göre ölçülebilir bir ek yüktür.
\textbf{Varsayılanınız \code{unique\_ptr} olsun}; sahiplik gerçekten
paylaşılmıyorsa \code{shared\_ptr}'a geçmeyin. Bir nesneye yalnızca
\emph{bakıyorsanız} (sahiplenmiyorsanız) ham işaretçi \code{T*} veya
referans \code{T\&} geçmek, \code{shared\_ptr} kopyalamaktan hem daha hızlı
hem daha dürüst bir niyet ifadesidir.}

\gsub{Yanlış: sıcak döngüde binlerce küçük tahsis}
...
void simulate_wrong(int frames) {
    for (int f = 0; f < frames; ++f) {
        std::vector<std::unique_ptr<Particle>> ps;
        for (int i = 0; i < 10000; ++i)
            ps.push_back(std::make_unique<Particle>());  // her kare 10.000 NEW
        // ... isle ...
    }   // her kare 10.000 tahsis + 10.000 serbest birakma -> cok yavas
}
\end{lstlisting}

\gsub{Doğru: tamponu yeniden kullan (havuz / arena)}
...
void simulate_right(int frames) {
    std::vector<Particle> ps;      // TEK, yeniden kullanilabilir tampon
    ps.reserve(10000);             // bir kez tahsis et

    for (int f = 0; f < frames; ++f) {
        ps.clear();                // kapasiteyi KORUR, bellegi serbest BIRAKMAZ
        for (int i = 0; i < 10000; ++i)
            ps.push_back({0.f, 0.f, 1.f});   // tahsis YOK, mevcut yere yaz
        // ... isle ...
    }   // tum simulasyon boyunca yalnizca 1 (belki birkac) tahsis
}
\end{lstlisting}

\notk[Basit nesne havuzu]{Nesneleri değil de \emph{tamponu} yeniden
kullanmak (yukarıdaki \code{clear} + yeniden doldurma) en basit ve en etkili
havuz (pool) tekniğidir. Daha karmaşık senaryolarda (nesnelerin ömürleri
serpiştirilmişse) bir \emph{serbest liste} (free list) ya da
\emph{arena/bump allocator} kullanılır: büyük bir blok bir kez ayrılır,
tahsisler yalnızca bir işaretçiyi ilerletir, hepsi birlikte serbest
bırakılır. Amaç hep aynıdır: \textbf{tahsis sayısını azaltmak.}}

\begin{lstlisting}
#include <vector>
#include <cstddef>

// Cok basit bir "bump/arena" ayirici: tek blok, tahsis = isaretci ilerlet.
class Arena {
    std::vector<std::byte> buffer_;
    std::size_t offset_ = 0;
public:
    explicit Arena(std::size_t bytes) : buffer_(bytes) {}

    void* allocate(std::size_t n, std::size_t align = alignof(std::max_align_t)) {
        std::size_t p = (offset_ + align - 1) & ~(align - 1);  // hizala
        if (p + n > buffer_.size()) return nullptr;            // dolu
        offset_ = p + n;
        return buffer_.data() + p;      // tahsis = sadece offset ilerletmek
    }
    void reset() { offset_ = 0; }       // TUMUNU birden "serbest birak" (O(1))
};

int main() {
    Arena arena(1 << 20);               // 1 MB'lik tek blok
    int* a = static_cast<int*>(arena.allocate(sizeof(int)));
    int* b = static_cast<int*>(arena.allocate(sizeof(int)));
    if (a && b) { *a = 1; *b = 2; }
    arena.reset();                      // tum tahsisler tek hamlede gecersiz
    return 0;
}
\end{lstlisting}

\ipucu{Arena/bump ayırıcı, ``aynı anda oluşturulup aynı anda yok edilen''
çok sayıda kısa ömürlü nesne için idealdir (bir kare, bir istek, bir ayrıştırma
turu). Tahsis neredeyse bedavadır (bir toplama işlemi) ve serbest bırakma
$\mathcal{O}(1)$'dir (offset'i sıfırla). Standart kütüphanede
\code{std::pmr::monotonic\_buffer\_resource} bu kalıbı hazır sunar ve
\code{std::pmr} konteynerleriyle doğrudan kullanılabilir.}

% =====================================================================
\gsec{İşaretçi ve Kaynak Yönetimi: Klasik Hatalar}

Performansın altında bir de \emph{doğruluk} katmanı vardır: işaretçilerle
yapılan hatalar (sarkan işaretçi, iki kez silme, geçersiz kılınmış iteratör)
programı yavaşlatmaz --- \emph{çökertir} veya sessizce belleği bozar. Bu
hatalar en sık ham (\code{new}/\code{delete}) bellek yönetiminde ve
konteyner yeniden tahsislerinde ortaya çıkar.

\gsub{Yanlış: yerel değişkenin adresini döndürmek}

\begin{lstlisting}
#include <string>

// HATA: 'local' fonksiyon bitince yigin (stack) uzerinde YOK OLUR;
// donen isaretci SARKAN (dangling) olur -> use-after-return (UB).
const std::string* name_wrong() {
    std::string local = "gecici";
    return &local;              // local birazdan olecek!
}
// int& ref_wrong() { int x = 5; return x; }  // ayni hata, referansla
\end{lstlisting}

\gsub{Doğru: değeri döndür (ya da ömrü çağırana ait olsun)}

\begin{lstlisting}
#include <string>
#include <memory>

// 1) En basiti: DEGERI dondur (NRVO ile kopyasiz gelir)
std::string name_right() {
    std::string local = "gecici";
    return local;               // nesnenin OMRU cagirana tasinir
}

// 2) Obekte yasamasi gerekiyorsa sahipligi acikca devret
std::unique_ptr<std::string> make_name() {
    return std::make_unique<std::string>("obekte");
}   // sahiplik cagirana gecer; kimse unutamaz, kimse iki kez silemez
\end{lstlisting}

\uyari{\textbf{Yerel bir nesnenin adresini/referansını asla döndürmeyin.}
Fonksiyon dönünce yığın çerçevesi (stack frame) geçersiz olur; o adrese
erişim tanımsız davranıştır (use-after-return) ve genelde ``bazen çalışıp
bazen çökme'' gibi bulunması en zor hatalara yol açar. Derleyicinin
\code{-Wreturn-local-addr} (GCC/Clang) uyarısını ciddiye alın;
\code{-fsanitize=address} (AddressSanitizer) bu hataları çalışma zamanında
kesin olarak yakalar.}

\gsub{Yanlış: yeniden tahsis iteratörü/işaretçiyi geçersiz kılar}
...
int main() {
    ...
    int* p = &v[0];             // ilk elemani gosteren ham isaretci

    for (int i = 0; i < 1000; ++i)
        v.push_back(i);         // bir noktada vektor YENIDEN TAHSIS edilir:
                                // tum elemanlar YENI bloga tasinir, ESKI blok
                                // serbest birakilir -> p artik SARKAN!

    std::cout << *p << "\n";     // UB: gecersiz (silinmis) bellege erisim
    return 0;
}
\end{lstlisting}

\gsub{Doğru: yeniden tahsisi önle veya indeksle çalış}
...
int main() {
    std::vector<int> v{1, 2, 3};
    v.reserve(1003);            // 1) tahsisi ONCEDEN yap -> blok tasinmaz

    int* p = &v[0];             // reserve'den SONRA al -> gecerli kalir
    for (int i = 0; i < 1000; ++i)
        v.push_back(i);

    std::cout << *p << "\n";     // guvenli: blok yerinde kaldi

    // 2) Daha guvenlisi: ham isaretci yerine INDEKS sakla.
    std::size_t idx = 0;        // indeks yeniden tahsisten ETKILENMEZ
    std::cout << v[idx] << "\n";
    return 0;
}
\end{lstlisting}

\notk[Geçersizleşme kuralı]{\code{std::vector}'e ekleme yapmak (\code{push\_back},
\code{insert}) yeniden tahsis tetiklerse, o vektöre ait \emph{tüm}
işaretçiler, referanslar ve iteratörler geçersizleşir. \code{erase} ise
silinen konumdan sonrasını geçersiz kılar. Uzun ömürlü bir tutamağa
(handle) ihtiyacınız varsa ham işaretçi yerine \textbf{indeks} saklayın ya
da elemanları \code{std::deque}/\code{std::list} gibi ekleme sırasında
mevcut elemanları taşımayan bir konteynerde tutun.}

\gsub{Yanlış: ham işaretçiyle belirsiz sahiplik (çift silme / sızıntı)}

\begin{lstlisting}
#include <string>

void raw_wrong() {
    std::string* s = new std::string("veri");
    std::string* alias = s;     // kim sahip? BELIRSIZ.
    delete s;                   // s silindi
    delete alias;               // AYNI bellek IKINCI kez silindi -> UB (double free)

    std::string* leak = new std::string("kayip");
    // ... erken return / istisna ... delete unutuldu -> SIZINTI
}
\end{lstlisting}

\gsub{Doğru: sahipliği unique\_ptr ile tek ve net yap}
...
void raw_right() {
    auto s = std::make_unique<std::string>("veri");
    std::string* peek = s.get();   // GOZLEM icin ham isaretci (sahiplenmez)
    // 'peek' asla delete EDILMEZ -- sahibi s'dir, o da otomatik siler.

    // Sahipligi devretmek gerekirse: acikca TASI
    std::unique_ptr<std::string> other = std::move(s);
    // s artik bos; iki kez silme imkansiz, istisna olsa bile sizinti yok.
}   // kapsam bitti -> other otomatik siler
\end{lstlisting}

\bilgi{\textbf{Sahiplik/gözlem ayrımı:} Bir işaretçi ya bir kaynağın
\emph{sahibidir} (onu silmekle yükümlü) ya da ona yalnızca \emph{bakar}
(gözlemci). Sahipliği \code{unique\_ptr} (tekil) veya \code{shared\_ptr}
(paylaşımlı) ile ifade edin; gözlem için çıplak \code{T*} veya \code{T\&}
kullanın ama onları asla \code{delete} etmeyin. Bu ayrımı tipe kodladığınızda
çift silme ve sızıntı \emph{sınıfsal olarak} imkânsızlaşır --- ``kim
siliyor?'' sorusu bir daha sorulmaz.}

% =====================================================================
\gsec{Beşin Kuralı: Kaynak Yöneten Sınıfı Doğru Yazmak}

Kısım~5'te Beşin Kuralı'nı ilkece gördük; burada onu \emph{bellek yönetimi
hatası} açısından ele alalım. Bir sınıf \emph{ham} bir kaynağı (öbek
tamponu, dosya, soket) elle yönetiyorsa, derleyicinin ürettiği
\emph{varsayılan} kopya/taşıma işlemleri o kaynağı \textbf{sığ} (shallow)
kopyalar --- ve bu neredeyse her zaman çift silmeye veya sızıntıya yol açar.

\gsub{Yanlış: yalnızca yıkıcı (Üçün Kuralı ihlali)}
...
public:
    ...
    ~Buffer() { delete[] data_; }        // yikici VAR ama kopya/tasima YOK

    // Derleyici VARSAYILAN kopya yapiciyi uretir: SIG kopya!
    // b2, b1'in data_ ISARETCISINI kopyalar -> IKISI de ayni bloga bakar.
};

void bug() {
    Buffer b1(100);
    Buffer b2 = b1;   // sig kopya: b2.data_ == b1.data_
}                     // once b2 yikilir: delete[] data_
                      // sonra b1 yikilir: AYNI data_ TEKRAR delete[] -> UB!
\end{lstlisting}

\gsub{Doğru: beş özel fonksiyonu da tanımla}
...
public:
    ...
    // 2) KOPYA YAPICI: DERIN kopya -- yeni blok ayir, icerigi kopyala
    Buffer(const Buffer& o) : data_(new int[o.size_]), size_(o.size_) {
        std::copy(o.data_, o.data_ + size_, data_);
    }

    // 3) KOPYA ATAMA: copy-and-swap ile (self-atamaya ve istisnaya guvenli)
    Buffer& operator=(const Buffer& o) {
        Buffer tmp(o);          // once kopyala
        swap(tmp);              // sonra takas et -> eski kaynak tmp ile silinir
        return *this;
    }

    // 4) TASIMA YAPICI: kaynagi CAL, otekini bosalt (noexcept onemli!)
    Buffer(Buffer&& o) noexcept : data_(o.data_), size_(o.size_) {
        o.data_ = nullptr;      // o artik silecek bir sey sahiplenmiyor
        o.size_ = 0;
    }

    // 5) TASIMA ATAMA: mevcut kaynagi birak, otekininkini cal
    Buffer& operator=(Buffer&& o) noexcept {
        if (this != &o) {       // self-taSImaya karsi koru
            delete[] data_;     // kendi eski kaynagimi birak
...
\uyari{\textbf{Üçün/Beşin Kuralı:} Bir sınıf yıkıcı, kopya yapıcı veya kopya
atama operatöründen \emph{herhangi birini} elle tanımlıyorsa, büyük olasılıkla
\emph{beşini de} (artı taşıma yapıcı ve taşıma atama) doğru tanımlamalıdır.
Yalnızca yıkıcı yazıp gerisini derleyiciye bırakmak, sığ kopya yüzünden
çift silmeye yol açar. Taşıma işlemlerini \code{noexcept} yapmayı
\textbf{unutmayın}: \code{std::vector} gibi konteynerler, yeniden tahsis
sırasında yalnızca \emph{noexcept} taşımayı kullanır; aksi halde güvenlik
için \emph{kopyalamaya} düşerler ve tüm optimizasyonu kaybedersiniz.}

\gsub{Doğru (daha iyisi): Sıfırın Kuralı}

Yukarıdaki beş fonksiyonu \emph{elle} yazmak hataya açıktır. En iyi çözüm,
kaynağı zaten kendini yöneten bir üyeye (\code{std::vector},
\code{std::unique\_ptr}, \code{std::string}) devredip \emph{hiçbir} özel
fonksiyon yazmamaktır --- buna \textbf{Sıfırın Kuralı} (Rule of Zero) denir.

\begin{lstlisting}
#include <vector>
#include <cstddef>

// Ham int* yerine std::vector kullan: kopya, tasima ve yikim
// DOGRU sekilde OTOMATIK gelir. Bes fonksiyonun HICBIRINI yazmayiz.
class Buffer {
    std::vector<int> data_;        // kaynagi kendisi yonetir (RAII)
public:
    explicit Buffer(std::size_t n) : data_(n) {}
    std::size_t size() const { return data_.size(); }
    // ~Buffer, kopya, tasima: HEPSI derleyiciden, hepsi DOGRU.
};

int main() {
    Buffer a(100);
    Buffer b = a;              // derin kopya -- vector kopyalar (dogru)
    Buffer c = std::move(a);   // tasima -- vector tasinir (ucuz, dogru)
    return 0;
}
\end{lstlisting}

\ipucu{\textbf{Önce Sıfırın Kuralı'nı hedefleyin.} Sınıfınızın üyeleri
kendi kaynaklarını yönetiyorsa (akıllı işaretçiler ve standart konteynerler),
hiçbir özel üye fonksiyon yazmanıza gerek kalmaz; derleyicinin ürettikleri
zaten doğrudur. Beşin Kuralı'nı yalnızca gerçekten \emph{ham} bir kaynağı
(bir C API tanıtıcısı, elle \code{new}'lenen bellek) sarmaladığınız --- ve
bunu tek bir sınıfta izole ettiğiniz --- ender durumlarda uygulayın. Modern
C++'ta ``çıplak \code{new}/\code{delete} neredeyse hiç görünmez'' kuralının
pratik karşılığı budur.}

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.6cm} >{\raggedright\arraybackslash}p{8.4cm}@{}}
\toprule
\textbf{Kural} & \textbf{Ne zaman} \\
\midrule
\textbf{Sıfırın Kuralı} & Üyeler kendini yönetiyor (\code{vector}, \code{unique\_ptr}, \code{string}). Hiçbir özel fonksiyon yazma. \emph{Varsayılan tercih.} \\
\textbf{Üçün Kuralı} & Ham kaynak var; en az yıkıcı+kopya yapıcı+kopya atama gerekli (C++98 mirası). \\
\textbf{Beşin Kuralı} & Ham kaynak var; performans için taşıma yapıcı+taşıma atama da (\code{noexcept}) eklenir. \\
\bottomrule
\end{tabular}
\end{center}

% =====================================================================
\gsec{Özet: Bellek Optimizasyonu Kontrol Listesi}

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{5.6cm} >{\raggedright\arraybackslash}p{6.4cm}@{}}
\toprule
\textbf{Yanlış (yaygın hata)} & \textbf{Doğru (tercih)} \\
\midrule
\code{reserve}'süz döngüde \code{push\_back} & Boyut biliniyorsa önce \code{reserve(n)} \\
\code{for (auto x : kap)} (kopya) & \code{for (const auto\& x : kap)} \\
Geçici kurup \code{push\_back} & \code{emplace\_back(args...)} \\
\code{out = out + p} döngüde & \code{out.reserve(...)} + \code{out += p} \\
Salt okunur \code{std::string} parametresi & \code{std::string\_view} (ömre dikkat) \\
\code{return std::move(local)} & \code{return local} (RVO'ya bırak) \\
Saklamak için \code{const T\&} alıp kopyalamak & Değere al + \code{std::move} (sink) \\
\code{std::list} / \code{vector<ptr>} tarama & Bitişik \code{std::vector<T>} \\
Tüm alanları her döngüde gezmek & Sıcak/soğuk ayrımı (SoA) \\
Rastgele üye sırası (bol dolgu) & Büyükten küçüğe sırala \\
\code{shared\_ptr(new T)} & \code{make\_shared<T>(...)} \\
Her yerde \code{shared\_ptr} & Varsayılan \code{unique\_ptr}; gözlem için \code{T*}/\code{T\&} \\
Sıcak döngüde bin küçük \code{new} & Tamponu yeniden kullan / arena \\
Yerel değişkenin adresini döndürmek & Değeri döndür / \code{unique\_ptr} ile sahiplik devret \\
Yeniden tahsis sonrası eski işaretçi/iteratör & Önce \code{reserve} ya da indeks sakla \\
Ham işaretçide belirsiz sahiplik (double free) & \code{unique\_ptr}; gözlem için \code{.get()} \\
Ham kaynakta yalnızca yıkıcı & Beşin Kuralı (5 fonksiyon, \code{noexcept} taşıma) \\
Beş fonksiyonu elle yazmak & Sıfırın Kuralı: kaynağı üye tipe devret \\
\bottomrule
\end{tabular}
\end{center}

\bilgi{\textbf{Öncelik sırası:} (1) Gereksiz \emph{tahsisleri} yok edin ---
en büyük kazanç genelde buradadır. (2) Gereksiz \emph{kopyaları} yok edin
(referans, taşıma, \code{string\_view}). (3) Veriyi \emph{bitişik} ve
\emph{önbellek dostu} düzenleyin. (4) Yalnızca ölçüm gösterirse mikro
optimizasyona (hizalama, SoA, özel ayırıcı) inin. Her adımda profilleyiciyle
\emph{öncesi/sonrası} ölçün --- optimizasyon bir inanç değil, bir ölçüm
disiplinidir.}

% #####################################################################
\gpart{13}{Modern C++ (C++17 / C++20)}
% #####################################################################
