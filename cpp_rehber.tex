...
% =====================================================================
\gsec{Yıkıcı (Destructor)}

Yıkıcı, bir nesne yok olduğunda \emph{otomatik} çağrılan özel üye
fonksiyondur. Görevi, nesnenin sahip olduğu kaynakları (öbek belleği,
dosyalar, kilitler) serbest bırakmaktır. \code{\~SinifAdi()} biçiminde
yazılır; parametre almaz, değer döndürmez, aşırı yüklenemez.

\begin{lstlisting}
#include <iostream>
#include <cstring>

// Dinamik bir karakter dizisi yoneten basit bir sinif
class TextHolder {
    char* data_;
    std::size_t size_;
public:
    // Yapici: kaynak edin (obek bellegi ayIr)
    TextHolder(const char* s) {
        size_ = std::strlen(s);
        data_ = new char[size_ + 1];      // OBEK tahsisi
        std::strcpy(data_, s);
        std::cout << "Yapici: \"" << data_ << "\"\n";
    }

    // YIKICI: kaynagi serbest birak
    ~TextHolder() {
        std::cout << "Yikici: \"" << data_ << "\" siliniyor\n";
        delete[] data_;                    // OBEK bellegini geri ver
    }
...
int main() {
    {
        TextHolder m("merhaba");
        std::cout << "Icerik: " << m.get() << "\n";
    }   // kapsam bitti -> m'nin YIKICISI otomatik cagrIlIr
    std::cout << "Kapsam disi\n";
    return 0;
}
\end{lstlisting}

\uyari{Bir sınıf \code{new} ile kaynak ayırıyorsa, yıkıcısında mutlaka
serbest bırakmalıdır. Ayrıca, bir sınıf temel sınıf (base class) olarak
kullanılacak ve işaretçiyle silinecekse, yıkıcısı \code{virtual}
olmalıdır; aksi halde türetilmiş sınıfın yıkıcısı çağrılmaz ve kaynak
sızar: \code{virtual \~Taban() = default;}}

% =====================================================================
\gsec{Kopya Yapıcı ve Kopya Atama Operatörü}

Yukarıdaki \code{MetinTutucu} sınıfı tehlikeli bir hataya açık: derleyicinin
\emph{ürettiği} varsayılan kopya, işaretçiyi olduğu gibi kopyalar (sığ kopya,
shallow copy). Sonuç: iki nesne aynı öbek belleğini gösterir ve ikisi de
onu silmeye çalışır --- ikili silme ve çökme! Çözüm, \emph{derin kopya}
(deep copy) yapan kendi kopya işlemlerimizi yazmaktır.

\gsub{Problem: Sığ Kopya}

\begin{lstlisting}
// Eger kopya yapici yazmazsak, derleyicinin urettigi SIĞ kopya:
//   veri_ = digeri.veri_;   // AYNI adresi kopyalar!
//
// MetinTutucu a("test");
// MetinTutucu b = a;        // b.veri_ == a.veri_ (AYNI obek!)
// -> yikicida: once b siler, sonra a AYNI bellegi tekrar siler -> COKME
\end{lstlisting}

\gsub{Çözüm: Derin Kopya}
...
public:
    ...
    // KOPYA YAPICI: yeni, BAGIMSIZ bir obek tahsis edip icerigi kopyalar
    TextHolder(const TextHolder& other)
        : size_(other.size_), data_(new char[other.size_ + 1]) {
        std::strcpy(data_, other.data_);   // DERIN kopya
        std::cout << "Kopya yapici\n";
    }

    // KOPYA ATAMA OPERATORU: mevcut nesneye baska bir nesnenin kopyasInI ata
    TextHolder& operator=(const TextHolder& other) {
        std::cout << "Kopya atama\n";
        if (this == &other) return *this;   // KENDINE atama korumasi

        // 1) Yeni kaynagi once ayIr (istisna guvenligi icin eskiyi silmeden once)
        char* newData = new char[other.size_ + 1];
        std::strcpy(newData, other.data_);

        // 2) Eski kaynagi serbest birak
        delete[] data_;

        // 3) Yeni duruma gec
        data_ = newData;
        size_ = other.size_;
        return *this;                        // zincirleme icin *this don
    }
...
int main() {
    TextHolder a("merhaba");
    TextHolder b = a;          // KOPYA YAPICI (yeni nesne ilklendiriliyor)
    TextHolder c("gecici");
    c = a;                      // KOPYA ATAMA (var olan nesneye atanIyor)

    std::cout << a.get() << " " << b.get() << " " << c.get() << "\n";
    return 0;
}   // uc bagImsIz obek, uc ayrI silme -> guvenli
\end{lstlisting}

\bilgi{\textbf{Kopya yapıcı vs kopya atama farkı:} Kopya yapıcı, \emph{yeni}
bir nesneyi başka bir nesnenin kopyası olarak \emph{ilklendirir}
(\code{MetinTutucu b = a;}). Kopya atama ise \emph{zaten var olan} bir
nesneye başka bir nesnenin değerini \emph{atar} (\code{c = a;}). Atama
operatörü, önce nesnenin eski kaynağını temizlemek zorundadır; yapıcının
temizleyecek eski kaynağı yoktur.}

\uyari{Kopya atama operatöründe üç şey kritiktir: (1) \textbf{kendine
atama} kontrolü (\code{if (this == \&digeri)}), (2) eskiyi silmeden önce
yeniyi ayırarak \textbf{istisna güvenliği}, (3) zincirleme (\code{a = b = c})
için \code{*this} döndürmek. Bir sonraki bölümde bu üçünü de zarifçe
çözen ``copy-and-swap'' deyimini göreceğiz.}

% =====================================================================
\gsec{Taşıma Yapıcı ve Taşıma Atama Operatörü}

Kopyalama, yeni bir öbek ayırıp içeriği baytı baytına kopyalar --- pahalı.
Ama kaynak nesne zaten geçici (rvalue) ise ve birazdan yok olacaksa, onun
kaynağını \emph{çalmak} (işaretçiyi devralıp orijinalini boşaltmak) çok
daha ucuzdur. İşte \textbf{taşıma semantiği} budur. C++11 ile geldi ve
modern C++'ın performansının temel taşıdır.
...
public:
    ...
    // Kopya yapici (derin kopya -- pahalI)
    TextHolder(const TextHolder& d)
        : size_(d.size_), data_(new char[d.size_ + 1]) {
        std::strcpy(data_, d.data_);
        std::cout << "KOPYA yapici (pahali)\n";
    }

    // TASIMA YAPICI: kaynagi CAL, orijinali bosalt (ucuz, noexcept)
    TextHolder(TextHolder&& d) noexcept
        : data_(d.data_), size_(d.size_) {   // isaretciyi devral
        d.data_ = nullptr;                     // orijinali BOSALT
        d.size_ = 0;
        std::cout << "TASIMA yapici (ucuz)\n";
    }

    // TASIMA ATAMA OPERATORU
    TextHolder& operator=(TextHolder&& d) noexcept {
        std::cout << "TASIMA atama\n";
        if (this == &d) return *this;
        delete[] data_;          // eski kaynagI birak
        data_ = d.data_;         // yeni kaynagI CAL
        size_ = d.size_;
        d.data_ = nullptr;       // orijinali bosalt (yIkIcIsI guvenli olsun)
        d.size_ = 0;
        return *this;
    }
...
TextHolder produce() { return TextHolder("gecici veri"); }

int main() {
    TextHolder a("kaynak");

    TextHolder b = std::move(a);   // TASIMA yapici: a'nIn kaynagI b'ye gecer
    std::cout << "a = " << a.get() << ", b = " << b.get() << "\n";
    // a artik bos, b = "kaynak"

    TextHolder c = produce();         // gecici (rvalue) -> TASIMA yapici
    c = TextHolder("yeni");        // gecici atanIyor -> TASIMA atama

    return 0;
}
\end{lstlisting}

\ipucu{Taşıma işlemlerini \textbf{her zaman \code{noexcept}} işaretleyin.
Bunun kritik bir pratik sonucu vardır: \code{std::vector} yeniden boyutlanırken
elemanlarını taşımak ister, ama yalnızca taşıma yapıcı \code{noexcept}'se
taşır; değilse güvenlik için \emph{kopyalar}. Yani \code{noexcept} olmayan
bir taşıma yapıcı, \code{vector} büyüdüğünde sessizce performans kaybına
yol açar.}

\bilgi{\textbf{Taşıma sonrası durum:} Taşınan (kaynak) nesne yok edilmez;
``geçerli ama belirsiz'' durumda kalır. Yıkıcısının güvenle çalışabilmesi
için onu tutarlı bir boş duruma getirmelisiniz (yukarıda \code{veri\_ = nullptr}).
Böylece kaynağın yıkıcısı \code{delete[] nullptr} çağırır ki bu güvenlidir.}
