...
\midrule
id & Nesneye benzersiz kimlik; başka nesnelerden erişmek için. Örn: \kod{id: box} \\
...
Rectangle {
    id: button
    width: 120; height: 40
    color: mouse.pressed ? "darkblue" : "steelblue"   // Basiliyken koyulasir
    radius: 6
    ...
    MouseArea {
        ...
        onEntered: button.scale = 1.1  // Fare geldi
        onExited:  button.scale = 1.0  // Fare cikti
    }
}
\end{lstlisting}
...
Row {
    spacing: 8
    property alias label: labelText.text   // Ic Text'in text'ini disa ac
    property alias value: input.text       // Ic TextField'in text'ini ac

    Text { id: labelText; text: "Ad:" }
...
Item {
    ...
    function select(i, name) {
        box.itemSelected(i, name)     // Sinyali yay
    }
...
Rectangle {
    ...
    MouseArea {
        ...
        onClicked: panel.state = (panel.state === "expanded")
                                 ? "" : "expanded"
    }
}
\end{lstlisting}
...
Rectangle {
    ...
    MouseArea { anchors.fill: parent
                onClicked: box.state = box.state === "large" ? "" : "large" }
}
\end{lstlisting}
...
Rectangle {
    ...
    MouseArea {
        anchors.fill: parent
        onClicked: ball.x = ball.x === 0 ? 200 : 0   // x'i degistir, animasyon otomatik
    }
}
\end{lstlisting}
...
Rectangle {
    ...
    SequentialAnimation {
        id: anim
        NumberAnimation { target: square; property: "rotation"
                          to: 360; duration: 500 }
        ParallelAnimation {                        // Ayni anda:
            NumberAnimation { target: square; property: "scale"
                              to: 1.5; duration: 300 }
            ColorAnimation  { target: square; property: "color"
...
ListView {
    ...
    model: ListModel {
        ListElement { name: "Ali";  age: 25 }
        ListElement { name: "Ayse"; age: 30 }
        ListElement { name: "Veli"; age: 28 }
    }
...
// Ekle / sil / guncelle
function add(name) { list.append({ name: name, age: 0 }) }
function remove(i) { list.remove(i) }
function clear()   { list.clear() }
function update(i, newName) { list.setProperty(i, "name", newName) }
\end{lstlisting}
...
Item {
    Loader {
        ...
        source: currentPage === "settings" ? "SettingsPage.qml" : "HomePage.qml"
    }

    // veya satir ici bilesen yukle:
    Loader {
        sourceComponent: isOpen ? detailComponent : null
    }
    Component {
        id: detailComponent
...
ColumnLayout {
    ...
    Button {
        text: "Tikla"
        onClicked: statusLabel.text = "Butona basildi"
    }
    ...
    Label { id: statusLabel; text: "Hazir" }
}
\end{lstlisting}
