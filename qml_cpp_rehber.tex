...
\begin{lstlisting}
Q_PROPERTY(type name READ getter WRITE setter NOTIFY nameChanged)
\end{lstlisting}
...
public slots:
    ...
    {
        return QString::number(number, 'f', 2);   // "3.14" gibi
    }
};
\end{lstlisting}
...
Text {
    ...
    text: calc.format(calc.divide(calc.add(10, 5), 3))  // "5.00"
}
\end{lstlisting}
...
\begin{lstlisting}
Q_INVOKABLE QVariantMap userInfo(int id)
{
    QVariantMap m;
    m["id"]     = id;
    m["name"]   = QStringLiteral("Ayse");
    m["age"]    = 30;
    m["active"] = true;
...
\begin{lstlisting}[style=qmlstyle]
var u = backend.userInfo(42)
console.log(u.name, u.age)     // "Ayse 30"
\end{lstlisting}
...
public:
    ...
    Q_INVOKABLE void start(const QString &url)
    {
        ...
        for (int p = 0; p <= 100; p += 25)
            emit progress(p);         // Parametreli sinyal
        emit finished(url, true);   // Cok parametreli sinyal
    }

signals:
    void started(const QString &url);
    void progress(int percent);
    void finished(const QString &url, bool success);
};
\end{lstlisting}
...
Downloader {
    ...
    // "started" -> "onStarted"
    onStarted: (url) => console.log("Basladi:", url)

    // "progress" -> "onProgress"; parametre adi sinyaldeki ile ayni
    onProgress: (percent) => progressBar.value = percent / 100

    onFinished: (url, success) => {
        if (success)
...
Connections {
    ...
    function onFinished(url, success) {
...
//   (QML'de: Text { objectName: "statusText" })
QObject *label = root->findChild<QObject*>("statusText");
if (label)
...
Window {
    function showMessage(text) {
        console.log("QML fonksiyonu:", text)
        return "alindi: " + text
    }
}
\end{lstlisting}
...
QObject *root = engine.rootObjects().first();
QVariant result;
QMetaObject::invokeMethod(root, "showMessage",
    Q_RETURN_ARG(QVariant, result),
    Q_ARG(QVariant, QString("Selam QML")));
qDebug() << result;   // "alindi: Selam QML"
\end{lstlisting}
